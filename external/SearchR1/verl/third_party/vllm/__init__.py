...
elif (package_version or '').startswith('0.8.'):
    # RL_v11 currently uses the HF rollout backend. fsdp_workers imports the
    # vLLM sharding module at import time, but that path is not instantiated.
    # Keep imports alive without binding to incompatible 0.6.x wrappers.
    vllm_version = package_version

    class LLM:  # type: ignore[no-redef]
        def __init__(self, *args, **kwargs):
            raise RuntimeError("vLLM rollout is not enabled for RL_v11 with vllm 0.8.x; use actor_rollout_ref.rollout.name=hf.")

    class LLMEngine:  # type: ignore[no-redef]
        pass

    class parallel_state:  # type: ignore[no-redef]
        @staticmethod
        def initialize_parallel_state(*args, **kwargs):
            return None
else:
    raise ValueError(
        f'vllm version {package_version} not supported. Currently supported versions are 0.3.1, 0.4.2, 0.5.4, 0.6.3 and import-only 0.8.x.'
    )
