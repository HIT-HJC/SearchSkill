...
def get_dataset(config):
    jsonl_path = os.path.join(config.dataset_path, f'{config.data_split}.jsonl')
    parquet_path = os.path.join(config.dataset_path, f'{config.data_split}.parquet')
    ...
    if os.path.exists(jsonl_path):
        ...
        col = 'question' if 'question' in df.columns else ('query' if 'query' in df.columns else None)
        if col is None:
            raise ValueError(f"No 'question' or 'query' in {parquet_path}")
        return [{'question': q} for q in df[col].tolist()]
    else:
        raise FileNotFoundError(f"Neither jsonl nor parquet found in {config.dataset_path}")
...
if __name__ == '__main__':
    ...
    out_dir = config.dataset_path if hasattr(args, 'dataset_path') else '.'
    os.makedirs(out_dir, exist_ok=True)
    out_fp = os.path.join(out_dir, f"retrieval_{args.retrieval_method}_top{args.retrieval_topk}.jsonl")
    with open(out_fp, "w", encoding="utf-8") as f:
        for q, docs, scs in zip(input_query, results, scores):
            f.write(json.dumps({
                "question": q,
                "hits": [
                    {
                        ...
                        "score": float(s)
                    } for d, s in zip(docs, scs)
                ]
            }, ensure_ascii=False) + "\n")
    print(f"[INFO] saved: {out_fp}")
