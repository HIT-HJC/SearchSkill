...
if os.environ.get("SEARCHSKILL_FORCE_CUDA_VISIBLE", "0") == "1":
    cuda_visible = os.environ.get("CUDA_VISIBLE_DEVICES", "").strip()
    if cuda_visible in {"", "NoDevFiles"}:
        os.environ["CUDA_VISIBLE_DEVICES"] = os.environ.get("SEARCHSKILL_ALL_CUDA_VISIBLE", "0,1,2,3")
...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        import torch.distributed
        if os.environ.get("SEARCHSKILL_FORCE_CUDA_VISIBLE", "0") == "1":
            local_rank = int(os.environ.get("RAY_LOCAL_RANK") or os.environ.get("LOCAL_RANK") or "0")
            if torch.cuda.device_count() > 0:
                torch.cuda.set_device(local_rank % torch.cuda.device_count())
            print(
                "[searchskill_cuda] "
                f"role={role} rank={os.environ.get('RANK')} "
                f"local_rank={os.environ.get('RAY_LOCAL_RANK') or os.environ.get('LOCAL_RANK')} "
                f"cuda_visible={os.environ.get('CUDA_VISIBLE_DEVICES')} "
                f"current_device={torch.cuda.current_device() if torch.cuda.device_count() > 0 else None} "
