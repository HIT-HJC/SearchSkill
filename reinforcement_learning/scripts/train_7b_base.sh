#!/usr/bin/env bash
set -euo pipefail

export ROOT="${ROOT:-/online1/ycsc_chenkh/hitici_11/HJCproject/SearchSkill Code}"
export SEARCHSKILL_ROOT="${SEARCHSKILL_ROOT:-$ROOT}"
export GPUS=4
export CUDA_VISIBLE_DEVICES=0,1,2,3
export TRAIN_BATCH_SIZE=8
export VAL_BATCH_SIZE=80
export ROLLOUT_N=8
export ROLLOUT_BACKEND=hf
export PPO_MINI_BATCH_SIZE=32
export PPO_MICRO_BATCH_SIZE=4
export TOTAL_STEPS=151
export SAVE_FREQ=15
export TEST_FREQ=15
export VAL_BEFORE_TRAIN=false
export MODEL_PATH="$SEARCHSKILL_ROOT"/supervised_finetuning/models/stage2_7b_base_merged
export RUN_NAME=rl_7b_base_from_stage2_4gpu
export OUT_DIR="$SEARCHSKILL_ROOT"/reinforcement_learning/runs/rl_7b_base_from_stage2_4gpu
export DATA_DIR="$SEARCHSKILL_ROOT"/reinforcement_learning/data/policy_7b_base
export LOG_DIR="$SEARCHSKILL_ROOT"/reinforcement_learning/logs
export LR=1e-6
export KL=0.001
export RETRIEVER_HOST=gpu031
export RETRIEVER_PORT=8000

cd "$ROOT"
bash reinforcement_learning/scripts/launch_training.sh
