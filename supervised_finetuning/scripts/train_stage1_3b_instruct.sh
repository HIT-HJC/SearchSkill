...
MODEL_DIR="$SEARCHSKILL_ROOT"/supervised_finetuning/models/stage1_3b_instruct
...
BASE_MODEL=${HF_MODELS:?Set HF_MODELS to your local model root}/Qwen2.5-3B-Instruct
...
export MASTER_PORT="${MASTER_PORT:-29765}"
...
exec ${PYTHON_BIN:-python} -m torch.distributed.run \
  ...
  --truncate-side left \
  --learning-rate "${LEARNING_RATE:-7e-5}" \
  --num-train-epochs "${NUM_TRAIN_EPOCHS:-2.0}" \
  ...
  --answer-loss-weight "${ANSWER_LOSS_WEIGHT:-2.5}" \
